...
1. Within the `upload` package, create a `Worker` to upload our `Note` to `GitHub`
...
2. Submit work request to `WorkManager`
    1. Within `NoteDetailFragment.uploadNote()` remove the call to start our upload service
    2. Construct a `WorkRequest` for `NoteUploadWorker` using `OneTimeWorkRequestBuilder<NoteUploadWorker>().setInputData(NoteUploadWorker.buildInputData(note)).build()`
    3. Submit the request to `WorkManager` using `WorkManager.getInstance(requireContext()).enqueue(uploadWorkRequest)`
